...
本白皮書專注於系統的**功能規格、資料結構、API 介面**等技術實現細節，**不包含視覺風格設計規範**。

- ✅ **包含**: 資料庫結構、API 端點、業務邏輯、權限規則、部署流程
- ❌ **不包含**: UI 視覺設計、配色方案、排版樣式、動畫效果

如需重建系統，開發者可依據本文件實現完整功能，**視覺風格可自由設計**。
...
### V2.0.6 (2026-04-02)
- **銷貨輸入編輯功能實作**
  - 新增 `/api/sales/update` API 端點
  - 支援修改產品型號、名稱、數量、單價、成本
  - 即時計算金額、利潤、毛利率
  - 虧損品項檢查與警告
  - 僅限老闆/會計權限
- **LINE 回覆表功能增強**
  - 「提問與回覆事項」分為兩個獨立欄位
  - 資料庫新增 `reply_content` 欄位
  - 列表頁面同步更新顯示
- **銷貨資料修正**
  - 修正 DY-1150401-010 單號產品名稱、編號、成本、利潤
- **ERP-v2 同步機制取消**
  - 停止每 10 分鐘單向同步備份
- 白皮書 v4.5 更新
