...
資料庫位於 `db/company.db`，採 SQLite 3 WAL 模式，共 53 張資料表，由外部 Parser 腳本自動維護核心業務資料。
...
| 端點 | 方法 | 說明 |
|------|------|------|
| /api/pdf/quote/\<doc_no\> | GET | 報價單 PDF（預覽，瀏覽器內開啟） |
| /api/pdf/order/\<doc_no\> | GET | 訂單 PDF（含訂金/尾款明細） |
| /api/pdf/sales/\<invoice_no\> | GET | 銷貨單 PDF |
| /api/pdf/inventory-count/\<count_id\> | GET | 盤點表 PDF（含差異統計） |
...
系統提供四種商業文件的 PDF 生成與列印功能，使用 Python reportlab 套件在伺服器端產生 PDF，前端透過瀏覽器內建 PDF 檢視器預覽，可直接選擇印表機列印或下載檔案。

#### 支援單據

| 單據類型 | API 端點 | 來源資料表 | 前端按鈕位置 |
|---------|---------|----------|------------|
| 報價單 | `GET /api/pdf/quote/<doc_no>` | sales_documents + sales_document_items | 報價輸入 → 歷史列表 |
| 訂單 | `GET /api/pdf/order/<doc_no>` | sales_documents + sales_document_items | 銷貨輸入 → 匯入訂單列表 |
| 銷貨單 | `GET /api/pdf/sales/<invoice_no>` | sales_history | 銷貨輸入 → 近期銷貨紀錄 |
| 盤點表 | `GET /api/pdf/inventory-count/<count_id>` | inventory_count + inventory_count_items | 盤點作業 → 盤點詳情/審核區 |

#### PDF 版面結構

每張 PDF 統一使用 A4 尺寸，包含以下區塊：

1. **公司表頭**：品牌名（COSH 電腦舖）、公司名稱與統編、地區標示
2. **金色分隔線**
3. **文件標題與單號**
4. **欄位資訊**：客戶/供應商、日期、業務、付款方式等（三欄排列）
5. **品項明細表格**：深色表頭、交替底色、數字靠右對齊
6. **合計區塊**：靠右對齊（訂單含訂金/尾款，盤點含品項數/差異項）
7. **備註區**（如有）
8. **簽核欄**：依單據類型不同（主管/經辦/客戶 或 主管/盤點人）
9. **頁尾**：版權聲明

#### 技術架構

- **共用模組**：`pdf_utils.py` 提供 `build_header()`、`build_items_table()`、`build_totals_block()`、`build_signature_block()` 等共用函式
- **混合字型**：CJK 字元使用系統 CJK 字型（DroidSansFallback / PingFang / 微軟正黑體），Latin 字元使用 Helvetica，自動偵測作業系統選擇對應字型（Windows / macOS / Linux 皆支援）
- **前端互動**：點擊「列印」按鈕後以 `window.open()` 在新分頁開啟 PDF，瀏覽器 PDF 檢視器提供列印與下載功能
...
| v1.7 | 2026-04-03 | **PDF 列印功能 + 盤點刪除修復** |
...
| | | **銷貨單 PDF**：`GET /api/pdf/sales/<invoice_no>`，依 invoice_no 聚合多筆品項，含品號/品名/數量/單價/金額 |
| | | **盤點表 PDF**：`GET /api/pdf/inventory-count/<count_id>`，顯示帳面/實際/差異/備註，統計品項數與差異項數 |
| | | **PDF 版面設計**：A4 尺寸，金色分隔線、深色表頭、交替底色、三欄簽核欄、頁尾版權聲明 |
