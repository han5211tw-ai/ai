...
| 資料庫資料表 | 61 張 |
| 後台管理頁面 | 6 個 |
| 程式碼行數（app.py） | 8,091 行 |
| 開發 Phase | 6 個 + QA 修正 + KPI 模組 + 財務模組 + 廠商管理 + 手機版優化 + 手機版導覽強化 + 智慧銷貨建議 + 操作日誌 + 電腦檢測系統 |
...
- **AI 幫手**：本地 oMLX 模型（gemma-4-e4b-it-8bit），全站浮動對話框，意圖分類引擎自動查詢 ERP 資料庫（銷貨/進貨/庫存/客戶/財務/班表/督導/獎金/盤點/外勤），支援多意圖合併與人名個人化查詢，登入後自動展開今日摘要（依角色區分老闆版/員工版）
...
資料庫位於 `db/company.db`，採 SQLite 3 WAL 模式，共 61 張資料表，由外部 Parser 腳本自動維護核心業務資料。
...
共 54 個頁面，分十個 Phase 開發，QA 階段新增 3 個。所有頁面繼承 base.html 母版。
...
「電腦檢測」置於「單據與需求」群組下，可見角色：老闆、會計、工程師、業務人員、業務部主管、門市部主管。
...
| | | **登入自動展開摘要**：前端 `_aiLoadDailySummary(user)` 登入成功後自動呼叫 daily-summary API，展開 AI 對話框並顯示今日摘要，失敗時靜默忽略 |
...
| 白皮書 | computershop-erp/ERP_v2_Whitepaper_v2.5.md |
| Parser 腳本 | ~/srv/parser/（禁止異動） |
