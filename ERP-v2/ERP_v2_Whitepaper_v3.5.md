...
| 資料庫資料表 | 66 張 |
| 後台管理頁面 | 6 個 |
| 程式碼行數（app.py） | 9,529 行 |
| 開發 Phase | 6 個 + QA 修正 + KPI 模組 + 財務模組 + 廠商管理 + 手機版優化 + 手機版導覽強化 + 智慧銷貨建議 + 操作日誌 + 電腦檢測系統 + 維修工單系統 |
...
- **電腦檢測系統**：218 個故障情境知識庫、AI 診斷對話（oMLX 本地模型）、故障樹瀏覽搜尋、檢測結論單信紙風格列印
- **維修工單系統**：六步驟漸進式流程（收件→檢測→報價→維修→取件→結案），A4 雙聯收件單（客戶聯＋店留聯）、正式維修單（含零件報價明細），客戶不修／無法維修結案路徑，銷貨輸入一鍵帶入維修單，統一工資品項 SV-LABOR
...
| 維修工單 | repair_orders, repair_order_items, repair_phrases |
...
| Phase 10 電腦檢測 | fault_diagnose.html | /fault_diagnose | 老闆/會計/工程師/業務人員/主管 |
| | print_repair_report.html | /print/repair-report | （列印專用，獨立 HTML） |
| Phase 11 維修工單 | repair_order.html | /repair_order | 老闆/會計/工程師/主管 |
...
| 端點 | 方法 | 說明 |
|------|------|------|
| /api/repair/create | POST | 建立維修工單（自動產生工單號 RO-{店碼}{YYYYMMDD}-{NNN}，預設狀態「檢測中」） |
...
「電腦檢測」與「維修知識庫」置於「維修檢測」獨立群組下，可見角色：老闆、會計、工程師、業務人員、業務部主管、門市部主管。

### 9.20 維修工單系統

六步驟漸進式維修流程，每一步完成後才解鎖下一步，搭配收件單與維修單兩種列印文件。
...
| 白皮書 | computershop-erp/ERP_v2_Whitepaper_v3.5.md |
| Parser 腳本 | ~/srv/parser/（禁止異動） |
